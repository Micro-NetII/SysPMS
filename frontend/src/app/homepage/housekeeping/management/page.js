...
export default function ManagementForm() {
  ...
  const fetchData = async () => {
    try {
      ...
      const combinedData = rooms.map((room) => {
        const housekeepingRecord = housekeeping.find(
          (h) => h.roomNumber === room.roomID
        );
        const reservationRecord = filteredReservations.find(
          (r) => r.roomNumber === parseInt(room.label)
        );

        const roomType = roomTypes.find(
          (rt) => rt.roomTypeID === room.roomType
        );
          ...
          if (checkInDate && checkOutDate) {
            ...
            if (
              ...
              if (isToday(parsedCheckInDate)) {
                ...
                isWithinInterval(currentDate, {
                  start: parsedCheckInDate,
                  end: parsedCheckOutDate,
                })
              ) {
              ...
              linenChangeIcon = determineLinenChangeIcon(
                checkInDate,
                checkOutDate
              );
            }
        ...
        return {
          ...room,
          state: housekeepingRecord
            ? stateOrder[housekeepingRecord.roomStatus - 1]
            : "outOfService",
  ...
  const filteredRoomData = roomData.filter((room) =>
    `${room.label} ${room.roomtypes.desc}`
      .toLowerCase()
  ...
  const handleAdd = async () => {
    try {
      const response = await axios.put(
        "/api/v1/housekeeping/maintenanceRooms",
        {
          data: modalData,
        }
      );
  ...
  const handleDragStart = (e, roomID) => {
    // Armazena o ID do quarto no objeto de dados do evento para acessar durante o drop
    console.log(`Drag Start - roomID: ${roomID}`);  // Log para verificar roomID
    e.dataTransfer.setData('roomID', roomID.toString()); // Garantir que o roomID seja uma string
  };
  ...
  const handleDrop = async (e, state) => {
    ...
    // Pega o roomID armazenado no dataTransfer
    const roomID = e.dataTransfer.getData('roomID');
  
    console.log(`Tentando mudar o quarto ${roomID} para o estado ${state}`);
  
    // Adicionando log para verificar o conteúdo de roomData
    console.log('Room Data:', roomData);
  
    // Localize o quarto pelo roomID (convertemos para número para comparação)
    const roomIndex = roomData.findIndex((room) => room.roomID === parseInt(roomID, 10));
    ...
    const newState = state; // O novo estado do quarto
    const roomNumber = roomData[roomIndex].roomID;
    const roomStatus = stateValues[newState]; // Mapeia o estado para o valor correto
    ...
    try {
      ...
      // Atualiza o estado local (roomData)
      const newRoomData = roomData.map((room, index) => {
        if (index === roomIndex) {
          return {
            ...room,
      ...
      setRoomData(newRoomData); // Atualiza o estado local
  
      // Loga a mudança de estado (opcional)
      await addHousekeepingLog(roomNumber, newState);
  
      console.log(`Estado do quarto ${roomNumber} atualizado para ${newState}`);
    } catch (error) {
      console.error("Error updating room status:", error);
    }
  };
...
const renderStates = (states, room) => {
  return states.map((state, index) => {
    const isOutOfService = state === "outOfService"; // Verifica se o estado é "fora de serviço"
    const stateCell = (
      <td
        key={state}
        className={`border border-tableCol select-none w-[150px] m-2 ${state === room.state ? stateColors[state] : ""}`}
        onDragOver={(e) => handleDragOver(e)} // Permite o drop
        onDrop={(e) => handleDrop(e, state)} // Trata o drop aqui
      >
        <div
          draggable={state === room.state} // Só permitir arrastar se o estado for igual
          onDragStart={(e) => handleDragStart(e, room.roomID)}
          onDragEnd={(e) => handleDragEnd(e)}
          onClick={() => isOutOfService && handleOutOfServiceClick(index)} // Adiciona o manipulador de clique
          style={{ cursor: isOutOfService ? 'pointer' : 'default' }} // Adiciona um cursor pointer se for "fora de serviço"
        >
          {state === room.state ? (
            <>
      ...
      <div className="bg-primary-600 py-5 max-h-16">
        <div className="flex justify-between">
          <p className="text-ml text-white px-4">
            <b>{t("housekeeping.management.managementTitle")}</b>
          </p>
          <div className="flex items-center gap-5 mr-4">
            <Button
              className=" bg-gradient-to-tr from-gray-50 to-gray-50 text-black shadow-lg justify-center"
              startContent={<RxFrame />}
            >
              {t("housekeeping.management.managementTitleButtonBulk")}
            </Button>
            <Button
              className="  bg-gradient-to-tr from-gray-50 to-gray-50 text-black shadow-lg justify-center"
              startContent={<IoFilter />}
            >
              {t("housekeeping.management.managementTitleButtonFilter")}
            </Button>
          </div>
        </div>
      </div>
            ...
            <tr>
              <th className="w-[15%] bg-tableCol text-left px-4">
                <Input
                  type="text"
                  placeholder={t(
                    "housekeeping.management.managementFilterRooms"
                  )}
                  ...
                  content={
                    <div className="px-1 py-2">
                      ...
                      <div className="text-tiny">
                        Number of people in the room on reservation
                      </div>
                    </div>
                  }
                >
                  ...
                  content={
                    <div className="px-1 py-2">
                      ...
                      <div className="text-tiny pb-2">
                        <FaArrowRight /> Check In
                      </div>
                      <div className="text-tiny pb-2">
                        <FaArrowLeft /> Check Out
                      </div>
                      <div className="text-tiny pb-2">
                        <MdOutlineAccessTime /> Stay
                      </div>
                      <div className="text-tiny pb-2">
                        <FaMinus /> No reservation
                      </div>
                    </div>
                  }
                >
              ...
              {stateOrder.map((state, stateIndex) => (
                <td
                  className="h-14 border border-tableCol select-none w-[150px]"
                  key={stateIndex}
                >
                  <div className="flex items-center justify-center text-center bg-white border border-grey p-2 rounded-lg">
                    {state === "outOfService" && <FaCircleMinus color="gray" />}
                    {state === "dirty" && <FaCircleXmark color="red" />}
                    {state === "touched" && <FaCircleExclamation color="orange" />}
                    {state === "cleaning" && <FaCircle color="#FFD700" />}
                    {state === "checked" && <FaQuestionCircle color="#00CED1" />}
                    {state === "clean" && <FaCheckCircle color="#0C8E6B" />}
                    <span className="text-sm uppercase">
                      {t(
                        `housekeeping.management.managementTable${state.charAt(0).toUpperCase() + state.slice(1)
                        }`
                      )}
                    </span>
                  </div>
